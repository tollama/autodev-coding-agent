...
Generate operator-ready ticket draft from autonomous failure artifacts:
```bash
autodev autonomous ticket-draft --run-dir ./generated_runs/<run_dir> --format markdown
...
For active AV6 kickoff planning, see `docs/AUTONOMOUS_V6_WAVE_PLAN.md` and `docs/AUTONOMOUS_V6_BACKLOG.md`.
...
- Autonomous quality gate snapshot API (`/api/autonomous/quality-gate/latest`) exposes latest gate/guard/preflight/operator guidance summary for operator triage
...
The Overview tab’s trust governance controls also now support explicit workflow actions for escalation management: operators can escalate immediately, clear escalation, or extend approval due dates without editing workflow fields manually.
Trust delivery also now has persisted attempt state and retry support. In addition to `stdout`, `log:`, and `webhook:` targets, the delivery path supports `ticket-json:`, `ticket-md:`, and `bundle-dir:` adapters, with retry-safe state exposed by `GET /api/autonomous/trust/delivery/state` and `POST /api/autonomous/trust/delivery/retry`.
The delivery path now also supports signed and retry-aware webhooks plus external-system payload adapters: `webhook-signed:`, `github-issue-json:`, `jira-ticket-json:`, and `notify-inbox-json:`. CLI/API callers can pass `webhook_secret`, `webhook_retry_limit`, and `webhook_timeout_sec` for richer webhook policy.
...
- `docs/PLAN_NEXT_WEEK.md` (AV6 execution active status + delivery framing)
- AV6 execution state is in progress (`av6.execution.in_progress`); canonical status board: `docs/STATUS_BOARD_CURRENT.md`
- `docs/AUTONOMOUS_V6_WAVE_PLAN.md` (AV6 goals, milestones, architecture deltas, risks)
- `docs/AUTONOMOUS_V6_BACKLOG.md` (AV6 prioritized execution backlog with DoD/test/PR splits)
- `docs/AUTONOMOUS_V6_WAVE_CHECKPOINT.md` (AV6 checkpoint scaffold)
...
- `docs/BACKLOG_NEXT_WEEK.md` (AV6 execution top queue quick reference)
