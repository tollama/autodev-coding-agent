...
- Failed outcomes now emit `.autodev/autonomous_incident_packet.json` using **incident packet minimal-field contract v2** (`schema_version=av5-005-v2`). Required fields are limited to operator-actionable minimum: run identity/status, typed+root-cause codes, routing ownership/severity/SLA/escalation, top operator action + playbook link, and reproduction pointers. Optional enrichment (for example retention/compaction decisions) remains additive. Successful outcomes keep no-op behavior (incident packet is not generated).
