...
| `av6.execution.in_progress` | Mode `AV6 Execution Active`; AV6 wave moves from kickoff packet publication into active delivery | Title `# PLAN — Next Wave (AV6 Execution Active)`; snapshot `- AV6 execution is in progress (...)` | Title `# BACKLOG — Next Wave (AV6 Active Delivery Queue)`; baseline `- AV6 execution: 🏗️ in progress (...)` | Status remains `✅ Closed on main` (AV4 closure ledger frozen) |
...
2. `av6.execution.in_progress` → delivery mode (ticket implementation and validation in flight)
...
> Canonical automation support is now guaranteed for `av6.kickoff.started` and `av6.execution.in_progress`.
> Keep `av6.stabilization.started` / `av6.closed` as draft-only until their docs lanes are finalized and promoted into the registry.
