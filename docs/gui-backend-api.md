...
## Trust Intelligence & Autonomous Endpoints

### `GET /api/autonomous/quality-gate/latest`

Latest autonomous quality gate snapshot (preflight/gate/guard/operator guidance).

### `GET /api/autonomous/trust/latest`

Latest trust summary and full trust intelligence packet for a run.

### `GET /api/autonomous/trust/trends`

Historical trust trend summary across recent runs.

- query params: `window` (default `10`)

### `GET /api/autonomous/trust/analytics`

Aggregated trust policy, review-reason, and governance analytics.

- query params: `window` (default `10`)

### `GET /api/autonomous/trust/model-eval`

Compare trust posture across recent models.

- query params: `window` (default `10`)

### `GET /api/autonomous/trust/inbox`

Operator inbox for review-required or policy-blocked trust items.

- query params: `window` (default `10`)

### `GET /api/autonomous/trust/events`

Recent trust and approval events for external consumers.

### `GET /api/autonomous/trust/approvals`

List recorded approval decisions for a run.

- query params: `run_id`

### `POST /api/autonomous/trust/approvals`

Record a trust approval, rejection, or acknowledgement for a run.

- body: `run_id`, `decision` (approve/reject/acknowledge), `reviewer`, `role`, `note`

### `GET /api/autonomous/trust/workflow`

Load trust approval workflow metadata for a run.

- query params: `run_id`

### `PATCH /api/autonomous/trust/workflow`

Update trust approval workflow assignment, due date, and escalation metadata.

- body: `run_id`, `assignees`, `due_at`, `current_owner`, `escalation_targets`, `notes`

### `POST /api/autonomous/trust/workflow/actions`

Apply a workflow action (escalate, clear escalation, extend due date).

- body: `run_id`, `action` (escalate/clear_escalation/extend_due), `actor`, `reason`, `extend_hours`, `due_at`

## Trust Delivery Endpoints

### `GET /api/autonomous/trust/delivery/preview`

Preview trust inbox, event, or run delivery payloads.

- query params: `mode` (inbox/events/run), `run_id`, `window`, `format`

### `POST /api/autonomous/trust/delivery/send`

Deliver trust inbox, event, or run payloads to configured targets.

- body: `mode`, `run_id`, `window`, `format`, `targets[]`, `dry_run`, `webhook_secret`, `webhook_retry_limit`, `webhook_timeout_sec`

### `GET /api/autonomous/trust/delivery/audit`

Inspect recent trust delivery activity and outcomes.

- query params: `window`

### `GET /api/autonomous/trust/delivery/state`

Inspect persisted trust delivery attempts and retry state.

- query params: `window`, `status`

### `POST /api/autonomous/trust/delivery/retry`

Retry failed targets from a previous trust delivery.

- body: `delivery_id`, `dry_run`, `webhook_secret`

### `GET /api/autonomous/browser-automation/latest`

Load the most recent persisted browser automation result.

## Compare Snapshot Endpoints

### `GET /api/runs/compare/snapshots`

List saved compare snapshots with filters and pagination.

- query params: `query`, `sort`, `archive_filter`, `pinned_filter`, `baseline_run_id`, `candidate_run_id`, `tag_filter`, `date_from`, `date_to`, `page`, `page_size`

### `POST /api/runs/compare/snapshots`

Persist a compare payload as a saved snapshot.

### `GET /api/runs/compare/snapshots/<snapshot_id>`

Open one saved compare snapshot.

### `PATCH /api/runs/compare/snapshots/<snapshot_id>`

Update compare snapshot metadata (display_name, pinned, archived, tags).

### `DELETE /api/runs/compare/snapshots/<snapshot_id>`

Delete a saved snapshot.

### `POST /api/runs/compare/snapshots/import`

Import an exported compare snapshot payload.

### `POST /api/runs/compare/snapshots/bulk`

Bulk metadata updates or deletes for saved snapshots.

### `POST /api/runs/compare/snapshots/retention/apply`

Preview or apply retention cleanup for saved snapshots.

## Additional Read Endpoints

### `GET /api/scorecard/latest`

Latest run scorecard summary.

### `GET /api/experiment-log`

Experiment log for latest or specific run.

### `GET /api/runs/<run_id>/experiment-log`

Experiment log for a specific run.

### `GET /api/processes`

List active/recent tracked processes with run linkage.

- query params: filtering options

### `GET /api/processes/<process_id>`

Get process details including state and transitions.

### `GET /api/processes/<process_id>/history`

Get process state transition history.

### `GET /api/docs/routes`

Get API reference documentation (self-describing route list).

### `GET /api/docs/deprecations/latest`

Get latest deprecated route usage activity.
...
The following POST helper routes remain as deprecated compatibility shims. Use the canonical REST item routes instead.

- `POST /api/runs/compare/snapshots/<snapshot_id>/rename` -> use `PATCH /api/runs/compare/snapshots/<snapshot_id>`
- `POST /api/runs/compare/snapshots/<snapshot_id>/metadata` -> use `PATCH /api/runs/compare/snapshots/<snapshot_id>`
- `POST /api/runs/compare/snapshots/<snapshot_id>/delete` -> use `DELETE /api/runs/compare/snapshots/<snapshot_id>`
