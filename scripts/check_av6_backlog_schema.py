#!/usr/bin/env python3
"""Validate AV6 backlog metadata table schema.
...
def validate(path: Path) -> list[ValidationError]:
    ...
    rows = _parse_table_rows(path.read_text(encoding="utf-8").splitlines())

    if len(rows) < 3:
        return [ValidationError(line=1, message="missing AV6 backlog metadata table")]
    ...
    for line_no, cells in rows[2:]:
        ...
        if not ID_RE.match(ticket_id):
            errors.append(ValidationError(line=line_no, message=f"invalid ID '{ticket_id}' (expected AV6-###)"))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate AV6 backlog metadata table schema")
    parser.add_argument(
        "--file",
        default="docs/AUTONOMOUS_V6_BACKLOG.md",
        help="Path to AV6 backlog markdown file",
    )
    ...
    if errors:
        print("[FAIL] AV6 backlog metadata schema check failed")
        for err in errors:
            print(f"  - {path}:{err.line}: {err.message}")
        return 1

    print("[PASS] AV6 backlog metadata schema check passed")
    return 0
